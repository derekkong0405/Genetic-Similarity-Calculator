...
#from setup_jaxSparse import adj_matrix_propagated, gene_encoder, N
...
def extract_gene_vectors(gene_names, gene_encoder = gene_encoder, adj_matrix = adj_matrix_propagated):
    ...
    for gene_name in gene_names:
        try:
            ...
            #gene_vector = adj_matrix[index].flatten() #only for jax sparse
...
def find_most_similar_genes_dynamic(query_gene, candidate_genes, dimension = N):
    ...
    sorted_idx, scores = jitted_similarity_core(query_vec, candidate_vecs)
...
gene_names = ["A", "TSC2", "BRCA1", "TSPY1","B","TSPYL1"]
similar_genes = find_most_similar_genes_dynamic("TSC1", gene_names)
print(f"Most similar to TSC1: {', '.join(f'{gene}({score:.5f})' for gene, score in similar_genes)}")
